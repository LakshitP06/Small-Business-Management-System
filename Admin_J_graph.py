...
def graph():
    ...
    if option1==1:
        ...
        for j in range(1,len(p)+1):
            list2.append(j)
        x=list2
        read1=pd.read_sql("select * from Jabal_Traders_Total",mydb,index_col='sno')
        ...
        if numb==1:
            ...
            plt.ylabel("INCOME")
            plt.title("INCOME GRAPH")
            month1="C:\\users\\laksh\\ip project\\Graphs_For_Jabal_Traders\\"+"Income"+' '+month+' '+year1+'.pdf'
            plt.savefig(month1)
            mycursor.execute("Truncate table Jabal_Traders_Total")
            mydb.commit()
            plt.stop()
            print()
            print('--------------------------------------------')
